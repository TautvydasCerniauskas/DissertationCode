...
class Sentence:
    def __init__(self, decoder_hidden, last_idx=SOS_token,
                 sentence_idxes=[], sentence_scores=[]):
    ...
    def avgScore(self):
        if len(self.sentence_scores) == 0:
            raise ValueError(
                "Calculate average score of sentence, but got no word")
    ...
    def addTopk(self, topi, topv, decoder_hidden, beam_size, voc):
        ...
        for i in range(beam_size):
            if topi[0][i] == EOS_token:
                terminates.append(
                    ([voc.index2word[idx.item()]
                      for idx in self.sentence_idxes] + ['<EOS>'],
...
def beam_decode(decoder, decoder_hidden, encoder_outputs,
    ...
    for i in range(max_length):
        for sentence in prev_top_sentences:
            ...
            term, top = sentence.addTopk(
                topi, topv, decoder_hidden, beam_size, voc)
    ...
    terminal_sentences += [sentence.toWordScore(voc)
                           for sentence in prev_top_sentences]
...
def evaluate(encoder, decoder, voc, sentence,
    ...
    if beam_size == 1:
        ...
        return beam_decode(decoder, decoder_hidden,
